...
class Model(object, metaclass=ModelMetaclass):
    ...
    def __init__(self, **kwargs):
        # 创建数据库连接
        self.conn = connect(host="localhost", port=3306, user="root", password="root", database="python_test", charset="utf8")
    ...
    def save(self):
        ...
        sql = "insert into %s (%s) values (%s);" % (self.__tablename__, ",".join(fields), ",".join(args_temp))
        print("*" * 50)
...
if __name__ == "__main__":
    ...
    user.save()
    user.find()
    user.update(("username", "password"), ("root", "1234"), 12345)
    user.delete(12345)
